...
function preload() {
    ...
    nexti = loadImage("next level.png")
}
...
function setup() {
    ...
    next = createSprite(500,450,50,50)
    next.addImage(nexti)
    next.visible = false 
    next.scale = 0.3

    v2butter = createSprite(920,330,50,50)
    v2butter.addImage(vbutteri);
    v2butter.scale = 0.15
    v2butter.visible = false
  ...
  if(gamestate === "next"){
    ...
    if(vbutter.x>900){
        vbutter.velocityX = -15
    }
    if(vbutter.x<12){
        vbutter.velocityX = 15
    }
    if(v2butter.x>900){
        v2butter.velocityX = -15
    }
    if(v2butter.x<12){
        v2butter.velocityX = 15
    }
    ...
    text("Use left right arrow keys and space to control the player",150,50)
    text("Note: DONT TOUCH THE BIG BUTTERFLY IT IS POSISONOUS",100,100)
    text("THIS TIME YOU ONLY HAVE TO GET 5 BUTTERFLIES",50,200)
    text("score: "+score,450,200)
    }
}
  if(gamestate === "win"){
